...
```ShellSession
$ podman run --rm -it  registry.suse.com/suse/helm:3.21 <helm-sub-command>
```

For instance, to display the Helm version, run:
```ShellSession
$ podman run --rm -it registry.suse.com/suse/helm:3.21 version --template='{{.Version}}'
v3.21
```
...
```ShellSession
$ podman run --rm -it --net=host -v /path/to/kubeconfig:/root/.kube/config:Z registry.suse.com/suse/helm:3.21
```
