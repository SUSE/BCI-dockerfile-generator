...
```ShellSession
$ podman run --rm -it  registry.suse.com/suse/helm:4.2 <helm-sub-command>
```

For instance, to display the Helm version, run:
```ShellSession
$ podman run --rm -it registry.suse.com/suse/helm:4.2 version --template='{{.Version}}'
v4.2
```
...
```ShellSession
$ podman run --rm -it --net=host -v /path/to/kubeconfig:/root/.kube/config:Z registry.suse.com/suse/helm:4.2
```
