...
class StagingBot:
    ...
    def from_github_comment(comment_text: str, osc_username: str) -> "StagingBot":
        ...
        # Created a staging project on OBS for 4: [home:pushman:BCI:Staging:SLE-15-SP4:sle15-sp4-HsmtR](url/to/proj)
...
"""
    ...
    async def write_all_image_build_recipes(
        ...
        for bci in self.bcis:
            ...
            files.append(
                [
                    f"{bci.package_name}/{fname}"
                    for fname in await bci.write_files_to_folder(str(img_dest_dir))
                ]
            )
...
"""
    ...
    async def _wait_for_all_pkg_service_runs(self) -> None:
        ...
        for pkg_name in self.package_names:
            await self._run_cmd(
                f"timeout 30m {self._osc} service wait {self.staging_project_name} {pkg_name}"
            )
