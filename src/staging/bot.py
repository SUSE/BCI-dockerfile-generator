...
"""
    ...
    async def write_all_image_build_recipes(
        ...
        for bci in self.bcis:
            ...
            async def delete_all_container_files(
                bci_pkg: BaseContainerImage, dest: str
            ) -> None:
            ...
            async def write_container_files(
                bci_pkg: BaseContainerImage, dest: str
            ) -> list[str]:
                return [
            ...
            await aiofiles.os.makedirs(img_dest_dir, exist_ok=True)
            if not bci.crate or bci.package_name not in obs_packages:
                await delete_all_container_files(bci, img_dest_dir)
            flattened_file_list.extend(await write_container_files(bci, img_dest_dir))

            if bci.crate and bci.package_name not in obs_packages:
                obs_packages.add(bci.package_name)

                async def write_crate_files(
                    crate: ContainerCrate, dest: str, container: BaseContainerImage
                ) -> list[str]:
                    flist = [
                        f"{container.package_name}/{fname}"
                        for fname in await crate.write_files_to_folder(dest, container)
                    ]
                    return flist

                flattened_file_list.extend(
                    await write_crate_files(bci.crate, img_dest_dir, bci)
                )
