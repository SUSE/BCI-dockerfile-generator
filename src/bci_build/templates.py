...
{% if image.packages %}{{ DOCKERFILE_RUN }} \\
    {% if image.from_target_image %}export PERMCTL_ALLOW_INSECURE_MODE_IF_NO_PROC=1; {% endif -%} zypper -n {%- if image.from_target_image %} --installroot /target --gpg-auto-import-keys {%- endif %} install {% if image.no_recommends %}--no-recommends {% endif %}{{ image.packages }}{%- if image.packages_to_delete %}; \\
