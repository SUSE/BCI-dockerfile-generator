...
class OsVersion(enum.Enum):
    ...
    #: SUSE Linux Container Collection - development stream
    SLCC_DEVELOPMENT = "SLCC-Development"
    #: SUSE Linux Container Collection - production stream
    SLCC_PRODUCTION = "SLCC-Production"
    #: SUSE Linux Container Collection - LTS stream
    SLCC_LTS = "SLCC-LTS"
    ...
    def is_slcc(self) -> bool:
        return self.value in (
            OsVersion.SLCC_DEVELOPMENT.value,
            OsVersion.SLCC_LTS.value,
            OsVersion.SLCC_PRODUCTION.value,
        )
    ...
    def deployment_branch_name(self) -> str:
        return f"sle15-sp{self.value}" if self.is_sles else str(self.value)
...
def _build_tag_prefix(os_version: OsVersion) -> str:
    ...
    if os_version.is_slcc:
        # SLCC uses SLCC-{PRODUCTION,DEVELOPMENT,LTS} => slcc/{production,development,lts}
        assert isinstance((stream_name := os_version.value), str)
        return "/".join(stream_name.lower().split("-"))
...
class BaseContainerImage(abc.ABC):
    ...
    @property
    def eula(self) -> str:
        """EULA covering this image. Is ``sle-eula`` for LTSS codestreams,
        otherwise ``sle-bci``.

        """
        return "sle-eula" if self.os_version.is_ltss else "sle-bci"
    ...
    def labelprefix(self) -> str:
        ...
        if self.os_version.is_opensuse:
            ...
            labelprefix = (
                "com.suse.slcc."
                + {
                    OsVersion.SLCC_DEVELOPMENT: "development",
                    OsVersion.SLCC_PRODUCTION: "production",
                    OsVersion.SLCC_LTS: "lts",
                }[self.os_version]
            )
