...
KUBEVIRT_EXCLUSIVE_ARCH = [Arch.X86_64, Arch.AARCH64]
...
KUBEVIRT_CONTAINERS = (
    ...
    + [
        ApplicationStackContainer(
            **_get_kubevirt_kwargs("handler", os_version, user="0", custom_end=False),
            ...
            custom_end=f"{DOCKERFILE_RUN} cp -f /usr/share/kube-virt/virt-handler/nssswitch.conf /etc/",
        )
        for os_version in (OsVersion.SL16_0, OsVersion.TUMBLEWEED)
    ]
    + [
    ...
    + [
        ApplicationStackContainer(
            **_get_kubevirt_kwargs("libguestfs-tools", os_version),
            package_list=sorted(
                [
    ...
    + [
        ApplicationStackContainer(
            **_get_kubevirt_kwargs("sidecar-shim", os_version),
            package_list=sorted(["kubevirt-sidecar-shim", "python3", "shadow"]),
            entrypoint=["/usr/bin/virt-sidecar-shim"],
        )
        for os_version in (OsVersion.SL16_0, OsVersion.TUMBLEWEED)
    ]
)
