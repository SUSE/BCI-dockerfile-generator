...
COPY third-party.gpg.key /tmp/{{ image.name }}.key
RUN rpm --import /tmp/{{ image.name }}.key
...
class ThirdPartyRepoMixin:
    ...
    def fetch_rpm_package(
        self, pkg: RpmPackage, latest: bool = True
    ) -> list[RpmPackage]:
        ...
        pkgs = self._repo_parser.query(
            name=pkg.name, version=pkg.version, latest=latest
        )
    ...
    def fetch_rpm_packages(self) -> list[RpmPackage]:
        ...
        if not self._rpms:
            for pkg in self.third_party_package_list:
                if isinstance(pkg, RpmPackage):
                    p = pkg
                else:
                    p = RpmPackage(name=pkg)
    ...
    def prepare_template(self) -> None:
        ...
        assert len(self.third_party_package_list) > 0, (
            "Third Party package list is empty"
        )
