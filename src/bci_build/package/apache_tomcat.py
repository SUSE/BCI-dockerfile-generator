...
TOMCAT_CONTAINERS = [
    ApplicationCollectionContainer(
        ...
        version=(tomcat_pkg_version := get_pkg_version(tomcat_pkg, os_version)),
        ...
        additional_versions=[
            f"{format_version(tomcat_pkg_version, ParseVersion.MINOR)}-jre{jre_version}",
            f"{format_version(tomcat_pkg_version, ParseVersion.PATCH)}-jre{jre_version}",
        ],
        cmd=[
        ...
        env={
            ...
            "TOMCAT_VERSION": tomcat_pkg_version,
            "CATALINA_HOME": (_CATALINA_HOME := "/usr/share/tomcat"),
            "CATALINA_BASE": _CATALINA_HOME,
            "PATH": f"{_CATALINA_HOME}/bin:$PATH",
        },
        custom_end=rf"""{generate_package_version_check(tomcat_pkg, tomcat_pkg_version, ParseVersion.PATCH)}
{DOCKERFILE_RUN} mkdir -p /var/log/tomcat; chown --recursive tomcat:tomcat /var/log/tomcat;
{DOCKERFILE_RUN} \
