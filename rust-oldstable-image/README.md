...
```Dockerfile
# Build the application using the Rust 1.97 container image
FROM registry.suse.com/bci/rust:1.97 as build
...
```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.suse.com/bci/rust:1.97 cargo build --release
```
