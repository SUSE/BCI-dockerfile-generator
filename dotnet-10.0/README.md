...
# final image
FROM registry.suse.com/bci/dotnet-runtime:10.0
WORKDIR /app
COPY --from=build /app .
...
```ShellSession
podman run --rm -v "$PWD":/app:Z -w /app registry.suse.com/bci/dotnet-sdk:10.0 dotnet run
```

To run unit tests in an isolated environment, use the following command:

```ShellSession
podman run --rm -v "$PWD":/app:Z -w /app/tests registry.suse.com/bci/dotnet-sdk:10.0 dotnet test --logger:trx
```
