# Go 1.26-openssl development container image
...
```Dockerfile
# Build the application using the Go 1.26-openssl development container image
FROM registry.suse.com/bci/golang:1.26-openssl as build
...
```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.suse.com/bci/golang:1.26-openssl go build -v
```

To run the application tests inside a container, use the following command:

```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.suse.com/bci/golang:1.26-openssl go test -v
```
