...
```ShellSession
$ podman run -it --rm registry.suse.com/suse/mariadb-client:12.3 mariadb -h $HOST_IP -u root -p
```

Use the command below to dump all databases:

```ShellSession
$ podman run -it --rm registry.suse.com/suse/mariadb-client:12.3 mariadb-dump $HOST_IP -p --all-databases > my-dump.sql
```
