...
```Dockerfile
FROM registry.suse.com/bci/gcc:16
WORKDIR /src/
COPY . /src/
...
```Dockerfile
FROM registry.suse.com/bci/gcc:16 as builder
WORKDIR /src/
COPY . /src/
...
```bash
podman run --rm -it -v $(pwd):/src/:Z registry.suse.com/bci/gcc:16 \
    gcc -o /src/app.out /src/*.c
```
or by invoking `make`
```bash
podman run --rm -it -v $(pwd):/src/:Z --workdir /src/ \
    registry.suse.com/bci/gcc:16 \
    make
```
