...
podman run -it --restart=always -p 5000:5000 -v /path/to/config.yml:/etc/registry/config.yml \
  -v /var/lib/docker-registry:/var/lib/docker-registry --name registry registry.suse.com/suse/registry:2.8
```
...
```
[Unit]
Description=SLE OCI Container Registry (Distribution)
