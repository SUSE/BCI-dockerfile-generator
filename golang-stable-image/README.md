# Go 1.27 development container image
...
operations through OpenSSL—will no longer be supported in SUSE Linux BCI.
...
```Dockerfile
# Build the application using the Go 1.27 development container image
...
```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.suse.com/bci/golang:1.27 go build -v
```

To run the application tests inside a container, use the following command:

```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.suse.com/bci/golang:1.27 go test -v
```
