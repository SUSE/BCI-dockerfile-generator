...
```Dockerfile
# Build the application using the Rust 1.98 container image
FROM registry.suse.com/bci/rust:1.98 as build
...
```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.suse.com/bci/rust:1.98 cargo build --release
```
